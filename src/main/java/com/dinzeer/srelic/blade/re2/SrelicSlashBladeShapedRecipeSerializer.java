...
public record SrelicSlashBladeShapedRecipeSerializer<T extends Recipe<?>, U extends T> (RecipeSerializer<T> compose,
    ...
    public U fromJson(@NotNull ResourceLocation id, @NotNull JsonObject json) {
        if (!json.has("result")) {
            ...
            object.addProperty("item", MODID+":"+ SRItem.SRELIC_BLADE_ID.getPath());
    ...
    public void toNetwork(@NotNull FriendlyByteBuf buf, @NotNull U recipe) {
        ...
        if (recipe instanceof SrelicSlashBladeShapedRecipe bladeRecipe) {
