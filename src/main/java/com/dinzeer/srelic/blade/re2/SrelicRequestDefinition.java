...
public class SrelicRequestDefinition {

    public static final Codec<SrelicRequestDefinition> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ResourceLocation.CODEC.optionalFieldOf("name", Srelic.prefix("none"))
                    .forGetter(SrelicRequestDefinition::getName),
            Codec.INT.optionalFieldOf("proud_soul", 0).forGetter(SrelicRequestDefinition::getProudSoulCount),
            Codec.INT.optionalFieldOf("kill", 0).forGetter(SrelicRequestDefinition::getKillCount),
            Codec.INT.optionalFieldOf("refine", 0).forGetter(SrelicRequestDefinition::getRefineCount),
            EnchantmentDefinition.CODEC.listOf().optionalFieldOf("enchantments", Lists.newArrayList())
                    .forGetter(SrelicRequestDefinition::getEnchantments),
            SwordType.CODEC.listOf().optionalFieldOf("sword_type", Lists.newArrayList())
                    .forGetter(SrelicRequestDefinition::getDefaultType))
            .apply(instance, SrelicRequestDefinition::new));
    ...
    public SrelicRequestDefinition(ResourceLocation name, int proud, int kill, int refine,
    ...
    public static SrelicRequestDefinition fromNetwork(FriendlyByteBuf buffer) {
        ...
        return new SrelicRequestDefinition(name, proud, kill, refine, enchantments, types);
    }

    public void initItemStack(ItemStack blade) {
        blade.getCapability(ISrelicblade.BLADESTATE).ifPresent(state -> {
            if (!this.name.equals(Srelic.prefix("none")))
    ...
    public boolean test(ItemStack blade) {
        if (blade == null || blade.isEmpty())
            return false;
        if (!blade.getCapability(ISrelicblade.BLADESTATE).isPresent())
            return false;
        var state = blade.getCapability(ISrelicblade.BLADESTATE).orElseThrow(NullPointerException::new);
        boolean nameCheck;
        if (this.name.equals(Srelic.prefix("none"))) {
    ...
    public static class Builder {
        ...
        public SrelicRequestDefinition build() {
            return new SrelicRequestDefinition(name, proudCount, killCount, refineCount, enchantments, defaultType);
        }
    }
}
